...
var FF3 = (function(window, $, module, undefined) {
 ...
 /*   function shuffleJobs() {
        ...
        for(var i=0;i<jobs_pool.length;i++) {
			...
			if (magJobsInit.includes(jobs_pool[i]))
				magJobsFinal.push(i);
        };
        ...
        for(var i=0;i<jobs_pool.length;i++) {
			...
			while ($('#chk-jobs-1st').is(':checked') && i == 1 && !magJobsFinal.includes(next))
    ...
    module.randomizeROM = function(buffer) {
		...
		if ($('#chk-bal-mini-toad-fix').is(':checked')) {
            module.applyPatch("toad_for_all", ROM);
            module.applyPatch("mini_for_all", ROM);
        };
